...
for n in range(0,Nt):
    ...
    if(n%10==0):
        print('Progress: ' + str(n) +'('+ str(n/Nt*100) + ')' + '%')
    ...
    Enorm[n] = 0.5*np.trapz((E*E),x=X)*eps0
...
electrEnergy_file = open('Electrostatic energy - k = 0.4.txt', 'w')

for value in Enorm:
    electrEnergy_file.write(str(value[0]) + '\n')
